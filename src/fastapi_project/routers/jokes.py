...
async def get_random_joke(service: JokeServiceDep, tag: str | None = None):
    return await service.get_random_joke(tag)


@router.get("/{joke_id}", response_model=JokeRead)
async def get_joke_by_id(service: JokeServiceDep, joke_id: uuid.UUID):
...
async def update_joke(service: JokeServiceDep, joke_id: uuid.UUID, payload: JokeUpdate):
    return await service.update_joke(joke_id, payload)


@router.delete("/{joke_id}", status_code=204)
async def delete_joke(service: JokeServiceDep, joke_id: uuid.UUID):
